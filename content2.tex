...
\section{Budowanie i weryfikacja modeli}
\label{cha:drugiDokument}


Z uwagi na specyfikę przygotowania danych treningowych i testowych, poprawność modeli została zwerifikowana według jednego z rodzajów walidacji krzyżowej --  \textit{Monte Carlo Cross Validation} (MCCV).
W tym podejściu pełne dane są losowo dzielone na dwa zbiory: treningowy i testowy. Na tak przygotowanych zbiorach dokonywane jest ucznie i testowanie modelu. Cykl ten jest powtarzany określoną przez badacza liczbę razy. Następnie wyniki każdego z eksperymentów są uśredniane. Podział danych przy kolejnych iteracjach procesu walidacji obrazuje rysunek \ref{fig:mccv}. Metoda ta została zastosowana dla każdego z badanych zestawów cech wideo z tabeli 3.2 oraz każdego z testowanych algorytmów.


\begin{center}
	\includegraphics[ height=8cm, width=12cm]{mccv}
	\captionof{figure}{opis \cite{mccv_fig}.}
	\label{fig:mccv}
	\label{fig:xccs}
\end{center}


Wybór tej metody był motywowany próbą uniezależnienia modelu od sposobu podziału danych na dane testowe i treningowe. Porównując do  metody \textit{k -Fold Cross Validation} zapobiega on sytuacji kiedy podgrupy są wydzielone w taki sposób, że na dana podgrupę przypadają próbki tylko z jeden z baz. Co więcej MCCV również pozwala na wykorzystanie wszystkich bądź znacznej większości danych do trenowania \cite{mccv}.  
...
Dla sieci neuronowych ustawiono maksymalną liczbę iteracji algorytmu $Backward$ $Propagation$, po której model będzie gotowy na 5000. Parametr $learning$ $rate$, określający szybkość uczenia, został ustawiony na wartość 0,001. Natomiast funkcję aktywacji stanowiła funkcja ReLU.\par

\vspace{5mm}

Podczas przedstawionego tu eksperymentu zostało stworzonych 20 160 modeli. Na tą liczbę składa się: 24 (20-dla SVR) -- kombinacje parametrów na jeden algorytm, 4 -- algorytmy uczenia maszynowego, 3 -- zestawy cech, 70 -- modeli biorących udział w walidacji krzyżowej.    
