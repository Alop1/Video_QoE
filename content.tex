...
Można wyróżnić dwa rodzaje kompresji: stratna i bezstratna. W przetwarzaniu obrazów stosowana jest ta pierwsza. Kompresja stratna znacząco obniża rozmiar pliku. W przypadku wideo wskaźnik kompresji może wynosić nawet 200:1 \todo[inline]{Skąd ta liczba?}. Technika ta korzysta wtedy z ograniczeń ludzkiego oka i usuwa informacje o obrazie, które i tak nie zostałyby wychwycone \todo[inline]{To po co algorytmy testowania jakości skoro nikt tego nie wychwytuje?}. Poniżej zaprezentowano schemat przetwarzania obrazu stałego kodekiem JPEG.
...
\begin{figure}[h]
		...
		\label{fig:xccs}
\end{figure}
\todo[inline]{Do takiego schematu polecam tikz-a - będzie lepiej wyglądać i łatwiej będzie go edytować. Tu są przykład:} \url{https://www.reddit.com/r/LaTeX/comments/8eu450/having_problems_creating_a_flowchart_using_tikz}
Większość kodeków bazuje swoje działania na transformacji DCT (\emph{ang. Discret Cosinus Transformation}). Polega ona na przeniesieniu, wcześniej wydzielonych z oryginalnego obrazu bloków pikseli, z dziedziny przestrzeni \todo[inline]{Co to jest dziedzina przestrzeni?} do dziedziny częstotliwości. Na tak przygotowanych blokach dokonywana jest kwantyzacja.Proces ten polega na zapisaniu  wartości pikseli na ograniczonej liczbie bitów, co już zmniejsza rozmiar danych. Kolejnym krokiem jest zastosowanie algorytmów kompresji \emph{run-lenght coding} i \emph{variable-lenght coding}. Pierwszy z nich grupuje ciągi zer i zapisuje ich liczbę zamiast całej takiej serii. Drugi zmienia postać słów kodowych na bardziej wydajna formę, poprze zapisanie tych często występujących krótszą sekwencją bitów \cite{kodowanie} (na przykład dzięki kodowaniu Huffman'a). \todo[inline]{Ten proces kompresja stratna obrazu i kompresja bezstratna bitów jakie na koniec otrzymamy, ten opis jest bardzo niedokładny}

Kodeki wideo dodatkowo dokonują kompresji na podstawie różnic w ramkach. Różnice te są określane jako wektor ruchu (\emph{ang. motion vector}) i liczone na zasadzie porównania makro-bloku z ramki referencyjnej i tego samego bloku z obecnej ramki \ref{fig:ramki}. Dzięki tej technice po kompresji zatrzymane są  pełne dane tylko o części ramek. Do odkodowania pozostałych wystarczą wektory ruchu \cite{kodowanie}. \todo[inline]{Nie jest to takie proste, kodujemy różnicę, więc same wektory ruchu nie wystarczą}
...
\todo[inline]{Ostatni akapit jest bardzo niezgrabny stylistycznie/gramatycznie. Teraz dowiadujemy się więcej o kompresji, ale nadal nie wiemy tego co najważniejsze, jak wyglądają typowe zniekształcenia, co mają wykrywać nasze metryki?}
...
\begin{itemize}[label=$\bullet$]
\item Blokowość ({\em ang. Blockiness}) -- powstaje podczas kodowania. Dotyczy związanych z tym procesem bloków pikseli i kompresji jaka jest na nich dokonywana. Objawia się poprzez zauważalną między tymi blokami\cite{blockiness}. \todo[inline]{Czegoś w tym zdaniu brakuje} Skala przyjęta w pracy: 0-3570. Im większa wartość, tym mniej widoczne zakłócenie. Dla obrazu bez zakłóceń około 0.9-1.01\cite{agh_vqm}. \todo[inline]{Nic z tego opisu nie rozumiem. Skala jest to 3570, ale już od 0.9 nic nie widać, to po co wartości > 1?}
\begin{figure}[h]
