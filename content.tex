...
Jedną z koncepcji poprawy trafności oceny wideo odnosi się do wykorzystania z gałęzi statystyki -- Uczenia maszynowego. Pomaga on odnajdywać zależności, nawet te na pozór nie intuicyjna, pomiędzy informacjami z danego wideo. Tą metodę zastosowano do tej pory podczas tworzenia metryk:x, y, z. 
...
\begin{itemize}[label=$\bullet$]
\item PSNR ({\em ang. Peak Signal-to-Noise Ratio}) -- prosta do policzenia i również często stosowana z pomiędzy metryk typu FR. Przykładowa implementacja dla PSNR \ref{eqn:somelabel} (gdzie MSE jest błędem średnio kwadratowym dla ramki, a $L$ maksymalną wartością sygnału). W pierwszej kolejności liczone jest zniekształcenie dla wszystkich ramek w stosunku do ich oryginału. Zazwyczaj do obliczań wykorzystuje się wyłącznie informację o luminacji piksela. Później następuje uśrednienie wyniku\cite{fr_psnr}. 
\begin{equation}
\label{eqn:somelabel}
...
\item SSIM ({\em ang. Structural Similarity Index}) -- jest jedną z najczęściej stosowanych metryk oceny jakości wideo. Również, jak poprzednia metryka, jest ona prosta w obliczeniach. Uwzględnia w swojej funkcji luminacje, kontrast i strukturę obu \ref{eqn:ssim}, gdzie S jest blokiem pikseli z ramki z oryginalnej sekwencji wideo, a S' jej zniekształconą wersją. $SSIM \in <0,1>$. 1 oznacza brak zniekształceń, a 0 idealne niedopasowanie \cite{ssim_1}\cite{ssim_2}.
\begin{equation}
\label{eqn:ssim}
